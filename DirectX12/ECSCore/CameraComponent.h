...
public:
	...
	float4 GetQuaternion() const {
		...
		double trace = r_Right.x + r_Up.y + r_Look.z;
		if (trace > 0.0) {
			...
			if (r_Right.x > r_Up.y && r_Right.x > r_Look.z) {
				double s = 2.0 * sqrt(1.0 + r_Right.x - r_Up.y - r_Look.z);
				...
				double s = 2.0 * sqrt(1.0 + r_Up.y - r_Right.x - r_Look.z);
				...
				double s = 2.0 * sqrt(1.0 + r_Look.z - r_Right.x - r_Up.y);
