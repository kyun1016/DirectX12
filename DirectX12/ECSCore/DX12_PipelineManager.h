...
public:
    ...
    struct CompiledPipeline {
        ...
        // Root Parameter 매핑 정보 (자동 바인딩용)
        struct RootParameterBinding {
            UINT rootParameterIndex;
            D3D12_ROOT_PARAMETER_TYPE type;
            UINT registerSlot;
            UINT registerSpace;
            std::string resourceName; // 셰이더에서 실제 사용하는 리소스 이름
        };
...
public:
    ...
    // 자동 Root Parameter 바인딩을 위한 함수들
    struct ResourceBinding {
        enum class Type { CBV, SRV, UAV, DescriptorTable };
        Type type;
        UINT rootParameterIndex;
        UINT registerSlot;
        UINT registerSpace;
    ...
    // 모든 바인딩 정보 조회 (디버깅용)
    std::vector<CompiledPipeline::RootParameterBinding> GetAllBindings(eRenderLayer layer) const {
        auto it = mLayerToPipeline.find(layer);
...
private:
    ...
    void RegisterSpritePipeline() {
        ...
        // Root Parameters 정의를 위한 멤버 변수 사용
        // auto& param = pipeline.rootParameters;
        // CD3DX12_ROOT_PARAMETER tmp;
    ...
    std::vector<CD3DX12_ROOT_PARAMETER> GenerateRootParametersFromShader(
        ...
        for (const auto& [bindPoint, bindDesc] : cbvBindings) {
            ...
            // 매핑 정보 저장
            CompiledPipeline::RootParameterBinding binding;
