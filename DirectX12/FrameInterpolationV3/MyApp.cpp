...
void MyApp::BuildPSOs()
{
	...
	shaderToyPsoDesc.DepthStencilState.DepthEnable = false;
...
void MyApp::CreateRtvAndDsvDescriptorHeaps(UINT numRTV, UINT numDSV, UINT numRTVST)
{
	AppBase::CreateRtvAndDsvDescriptorHeaps(APP_NUM_BACK_BUFFERS + RTV_USER_SIZE, 1 + MAX_LIGHTS, RTV_TOY_SIZE);
}
void MyApp::OnResize()
{
	AppBase::OnResize();
...
void MyApp::Render()
{
	...
	{
		D3D12_VIEWPORT viewport = { 0.0f, 0.0f, mCBShaderToy.iResolution.x, mCBShaderToy.iResolution.y, 0.0f, 1.0f };
		D3D12_RECT scissorRect = { 0, 0, (LONG)mCBShaderToy.iResolution.x, (LONG)mCBShaderToy.iResolution.y };
...
void MyApp::DrawShaderToy(int idx)
{
	...
	rtvHandle = mhCPUDescHandleST[idx];   // 새로 만든 RTV
	mCommandList->OMSetRenderTargets(1, &rtvHandle, false, nullptr);
	float clearColor[] = { 0, 0, 0, 0 };
	mCommandList->ClearRenderTargetView(mhCPUDescHandleST[idx], clearColor, 0, nullptr);
...
void MyApp::CopyRTV(const int srvIdx)
{
	// Indicate a state transition on the resource usage.
	D3D12_RESOURCE_BARRIER RenderBarrier =
		CD3DX12_RESOURCE_BARRIER::Transition(mSwapChainBuffer[mCurrBackBuffer].Get(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_COPY_SOURCE);
	D3D12_RESOURCE_BARRIER SRVUserBufBarrier =
		CD3DX12_RESOURCE_BARRIER::Transition(mSRVUserBuffer[srvIdx].Get(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_COPY_DEST);

	mCommandList->ResourceBarrier(1, &RenderBarrier);
	mCommandList->ResourceBarrier(1, &SRVUserBufBarrier);

	mCommandList->CopyResource(mSRVUserBuffer[srvIdx].Get(), mSwapChainBuffer[mCurrBackBuffer].Get());

	RenderBarrier.Transition.StateBefore = D3D12_RESOURCE_STATE_COPY_SOURCE;
	RenderBarrier.Transition.StateAfter = D3D12_RESOURCE_STATE_RENDER_TARGET;
	SRVUserBufBarrier.Transition.StateBefore = D3D12_RESOURCE_STATE_COPY_DEST;
	SRVUserBufBarrier.Transition.StateAfter = D3D12_RESOURCE_STATE_RENDER_TARGET;
	mCommandList->ResourceBarrier(1, &RenderBarrier);
	mCommandList->ResourceBarrier(1, &SRVUserBufBarrier);
}