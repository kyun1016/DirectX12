...
void MyApp::BuildShadersAndInputLayout()
{
	...
	mShaders["MainVS"] = D3DUtil::CompileShader(L"Main.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["MainPS"] = D3DUtil::CompileShader(L"Main.hlsl", defines, "PS", "ps_5_0");
	mShaders["AlphaTestedPS"] = D3DUtil::CompileShader(L"Main.hlsl", alphaTestDefines, "PS", "ps_5_0");

	mShaders["NormalVS"] = D3DUtil::CompileShader(L"Normal.hlsl", nullptr, "VS", "vs_5_0");
...
void MyApp::Render()
{
	...
	for (int i = 0; i < MAX_LAYER_DEPTH; ++i)
	{
		...
		else if (mLayerType[i] == RenderLayer::WaveVS_CS)
		{
			mCSWaves->UpdateWaves(mTimer, mCommandList.Get());
			
			mCommandList->SetGraphicsRootSignature(mCSWaves->GetRootSig());
